      ...
      (let* ((all-elements (loop for minor-body in minor-bodies collecting
        ...
        (let ((elements (compute-elements major-body (first minor-bodies) ref-plane-normal ref-direction)))
          (with-slots (minor-body
            ...
            (gl-place-string (format nil "PeT ~a" (periapsis-time elements))    (make-vector-2 0 1) :anchor-point (make-vector-2 0  9))
            (gl-place-string (format nil "T   ~a" (orbital-period elements))    (make-vector-2 0 1) :anchor-point (make-vector-2 0 10))
            (let ((per (periapsis-radius elements)) (apr (apoapsis-radius elements)) (rad (magnitude (sub (slot-value major-body 'pos) (slot-value minor-body 'pos)))))
              (if (eq (slot-value mfd 'dst-mode) 'radius)
                (progn
                  (gl-place-string (format nil "PeR ~a" per)  (make-vector-2 0 1) :anchor-point (make-vector-2 0 11))
                  (gl-place-string (format nil "ApR ~a" apr)   (make-vector-2 0 1) :anchor-point (make-vector-2 0 12))
                  (gl-place-string (format nil "Rad ~a" rad)   (make-vector-2 0 1) :anchor-point (make-vector-2 0 13)))
                (progn
                  (gl-place-string (format nil "PeA ~a" (- per (slot-value major-body 'radius)))  (make-vector-2 0 1) :anchor-point (make-vector-2 0 11))
                  (gl-place-string (format nil "ApA ~a" (when (not (null apr))
                                                        (- apr (slot-value major-body 'radius))))   (make-vector-2 0 1) :anchor-point (make-vector-2 0 12))
                  (gl-place-string (format nil "Alt ~a" (- rad (slot-value major-body 'radius)))  (make-vector-2 0 1) :anchor-point (make-vector-2 0 13)))))))
